...
def _setup_ivf_signatures(lib: ctypes.CDLL) -> None:
    """Declare C function signatures for IVF index API."""
    # Lifecycle
    lib.memscale_ivf_index_build.restype = ctypes.c_void_p
    lib.memscale_ivf_index_build.argtypes = [
        ctypes.c_char_p, ctypes.c_char_p, ctypes.c_char_p,
    ]

    lib.memscale_ivf_index_free.restype = None
    lib.memscale_ivf_index_free.argtypes = [ctypes.c_void_p]

    # Operations
    lib.memscale_ivf_index_add.restype = ctypes.c_int32
    lib.memscale_ivf_index_add.argtypes = [
        ctypes.c_void_p, ctypes.c_char_p, ctypes.c_char_p,
    ]

    lib.memscale_ivf_index_remove.restype = ctypes.c_int32
    lib.memscale_ivf_index_remove.argtypes = [
        ctypes.c_void_p, ctypes.c_char_p,
    ]

    lib.memscale_ivf_index_search.restype = ctypes.c_int32
    lib.memscale_ivf_index_search.argtypes = [
        ctypes.c_void_p, ctypes.c_char_p, ctypes.c_uint32,
        ctypes.POINTER(ctypes.c_char_p),
    ]

    lib.memscale_ivf_index_search_with_filter.restype = ctypes.c_int32
    lib.memscale_ivf_index_search_with_filter.argtypes = [
        ctypes.c_void_p, ctypes.c_char_p, ctypes.c_uint32,
        ctypes.c_char_p, ctypes.POINTER(ctypes.c_char_p),
    ]

    lib.memscale_ivf_index_batch_search.restype = ctypes.c_int32
    lib.memscale_ivf_index_batch_search.argtypes = [
        ctypes.c_void_p, ctypes.c_char_p, ctypes.c_uint32,
        ctypes.POINTER(ctypes.c_char_p),
    ]

    # Info
    lib.memscale_ivf_index_len.restype = ctypes.c_int32
    lib.memscale_ivf_index_len.argtypes = [
        ctypes.c_void_p, ctypes.POINTER(ctypes.c_int64),
    ]

    lib.memscale_ivf_index_dimension.restype = ctypes.c_int32
    lib.memscale_ivf_index_dimension.argtypes = [
        ctypes.c_void_p, ctypes.POINTER(ctypes.c_int64),
    ]

    lib.memscale_ivf_index_metrics.restype = ctypes.c_int32
    lib.memscale_ivf_index_metrics.argtypes = [
        ctypes.c_void_p, ctypes.POINTER(ctypes.c_char_p),
    ]


class MembrainIvfIndex:
    """Python client for the Membrain IVF (Inverted File) vector index.

    Partitions vectors into Voronoi cells using k-means clustering.
    At query time, only the nprobe closest cells are scanned.
    Best for datasets from 10k to 1M vectors.
    ...
    def build(
        ...
        *,
        ...
        Args:
            ...
            config: Optional IVF configuration with keys:
                - num_cells (int): Number of Voronoi cells (nlist).
                - nprobe (int): Cells to probe at query time.
        ...
        handle = lib.memscale_ivf_index_build(
            config_bytes, ids_bytes, vectors_bytes,
        )

        if not handle:
            err = get_last_error(lib)
            raise MembrainError(f"failed to build IVF index: {err}")
    ...
    def add(self, id: str, embedding: list[float]) -> None:
        ...
        code = self._lib.memscale_ivf_index_add(
    ...
    def remove(self, id: str) -> None:
        ...
        code = self._lib.memscale_ivf_index_remove(
    ...
    def search(self, query: list[float], k: int = 10) -> list[IndexSearchResult]:
        ...
        code = self._lib.memscale_ivf_index_search(
            self._handle,
            encode(query_json),
            ctypes.c_uint32(k),
            ctypes.byref(out),
    ...
    def search_with_filter(
        ...
        code = self._lib.memscale_ivf_index_search_with_filter(
            self._handle,
            encode(query_json),
            ctypes.c_uint32(k),
            encode(ids_json),
    ...
    def batch_search(
        ...
        code = self._lib.memscale_ivf_index_batch_search(
            self._handle,
            encode(queries_json),
            ctypes.c_uint32(k),
            ctypes.byref(out),
    ...
    def __len__(self) -> int:
        ...
        out = ctypes.c_int64()
        code = self._lib.memscale_ivf_index_len(
    ...
    def dimension(self) -> int:
        ...
        out = ctypes.c_int64()
        code = self._lib.memscale_ivf_index_dimension(
            self._handle, ctypes.byref(out),
        )
        check(self._lib, code)
        return out.value
    ...
    def metrics(self) -> IndexMetrics:
        ...
        out = ctypes.c_char_p()
        code = self._lib.memscale_ivf_index_metrics(
            self._handle, ctypes.byref(out),
        )
        check(self._lib, code)
        try:
    ...
    def close(self) -> None:
        ...
        if hasattr(self, '_handle') and self._handle:
            self._lib.memscale_ivf_index_free(self._handle)
            self._handle = None
    ...
    def __enter__(self) -> MembrainIvfIndex:
        return self
