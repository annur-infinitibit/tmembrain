"""Thread-safe concurrent Vamana vector index client."""
...
def _setup_concurrent_vamana_signatures(lib: ctypes.CDLL) -> None:
    """Declare C function signatures for concurrent Vamana index API."""
    # Lifecycle
    lib.memscale_concurrent_vamana_index_build.restype = ctypes.c_void_p
    lib.memscale_concurrent_vamana_index_build.argtypes = [
        ctypes.POINTER(ctypes.c_char_p), ctypes.POINTER(ctypes.c_float),
        ctypes.c_uint32, ctypes.c_uint32, ctypes.c_char_p,
    ]

    lib.memscale_concurrent_vamana_index_clone.restype = ctypes.c_void_p
    lib.memscale_concurrent_vamana_index_clone.argtypes = [ctypes.c_void_p]

    lib.memscale_concurrent_vamana_index_free.restype = None
    lib.memscale_concurrent_vamana_index_free.argtypes = [ctypes.c_void_p]

    # Operations (thread-safe)
    lib.memscale_concurrent_vamana_index_add.restype = ctypes.c_int32
    lib.memscale_concurrent_vamana_index_add.argtypes = [
        ctypes.c_void_p, ctypes.c_char_p, ctypes.POINTER(ctypes.c_float), ctypes.c_uint32,
    ]

    lib.memscale_concurrent_vamana_index_remove.restype = ctypes.c_int32
    lib.memscale_concurrent_vamana_index_remove.argtypes = [
        ctypes.c_void_p, ctypes.c_char_p, ctypes.POINTER(ctypes.c_int32),
    ]

    lib.memscale_concurrent_vamana_index_search.restype = ctypes.c_int32
    lib.memscale_concurrent_vamana_index_search.argtypes = [
        ctypes.c_void_p, ctypes.POINTER(ctypes.c_float), ctypes.c_uint32,
        ctypes.c_uint32, ctypes.POINTER(ctypes.c_char_p),
    ]

    # Info
    lib.memscale_concurrent_vamana_index_len.restype = ctypes.c_int32
    lib.memscale_concurrent_vamana_index_len.argtypes = [
        ctypes.c_void_p, ctypes.POINTER(ctypes.c_int64),
    ]

    lib.memscale_concurrent_vamana_index_dimension.restype = ctypes.c_int32
    lib.memscale_concurrent_vamana_index_dimension.argtypes = [
        ctypes.c_void_p, ctypes.POINTER(ctypes.c_int64),
    ]


class ConcurrentVamanaIndex:
    ...
    Usage:
        ...
        index = ConcurrentVamanaIndex.build(ids, vectors, dimension=128)
        ...
        *,
        ...
        Args:
            ...
            config: Optional configuration dictionary. Fields:
                "max_degree", "alpha", "l_build", "distance_metric".
            lib_path: Optional path to the shared library.

        Returns:
            A new ConcurrentVamanaIndex instance.
        """
        lib = load_library(lib_path)
        _setup_concurrent_vamana_signatures(lib)
        ...
        handle = lib.memscale_concurrent_vamana_index_build(
            id_pointers,
            vectors_array,
        ...
        if not handle:
            ...
            raise MembrainError(f"failed to build concurrent Vamana index: {err}")

        return cls(handle, lib)

    def clone(self) -> ConcurrentVamanaIndex:
        ...
        Returns:
            A new ConcurrentVamanaIndex instance sharing the same underlying index.
        """
        cloned_handle = self._lib.memscale_concurrent_vamana_index_clone(self._handle)
        if not cloned_handle:
            err = get_last_error(self._lib)
            raise MembrainError(f"failed to clone concurrent Vamana index: {err}")

        return ConcurrentVamanaIndex(cloned_handle, self._lib)
    ...
    def add(self, id: str, embedding: list[float]) -> None:
        ...
        code = self._lib.memscale_concurrent_vamana_index_add(
            self._handle,
            encode(id),
            vector_array,
    ...
    def remove(self, id: str) -> bool:
        ...
        code = self._lib.memscale_concurrent_vamana_index_remove(
            self._handle,
            encode(id),
            ctypes.byref(found),
        )
    ...
    def search(self, query: list[float], k: int = 10) -> list[IndexSearchResult]:
        ...
        code = self._lib.memscale_concurrent_vamana_index_search(
            self._handle,
            query_array,
    ...
    def __len__(self) -> int:
        ...
        out = ctypes.c_int64()
        code = self._lib.memscale_concurrent_vamana_index_len(
    ...
    def dimension(self) -> int:
        ...
        out = ctypes.c_int64()
        code = self._lib.memscale_concurrent_vamana_index_dimension(
            self._handle, ctypes.byref(out),
        )
        check(self._lib, code)
        return out.value
    ...
    def close(self) -> None:
        ...
        if hasattr(self, '_handle') and self._handle:
            self._lib.memscale_concurrent_vamana_index_free(self._handle)
            self._handle = None
    ...
    def __enter__(self) -> ConcurrentVamanaIndex:
        return self
