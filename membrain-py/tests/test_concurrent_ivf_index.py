...
def test_concurrent_ivf_index_build():
    """Test building a concurrent IVF index."""
    ids = [str(uuid.uuid4()) for _ in range(100)]
    vectors = [[float(i % 3)] * 3 for i in range(100)]

    index = ConcurrentIvfIndex.build(ids, vectors, dimension=3)
    assert index.dimension == 3
    assert len(index) == 100
    index.close()


def test_concurrent_ivf_index_search():
    """Test searching in a concurrent IVF index."""
    ids = [str(uuid.uuid4()) for _ in range(100)]
    vectors = [[float(i % 3)] * 3 for i in range(100)]
...
def test_concurrent_ivf_index_add_and_remove():
...
def test_concurrent_ivf_index_clone():
...
def test_concurrent_ivf_index_multi_threaded_search():
    """Test concurrent searches from multiple threads."""
    ids = [str(uuid.uuid4()) for _ in range(100)]
    vectors = [[float(i % 16)] * 16 for i in range(100)]

    index = ConcurrentIvfIndex.build(ids, vectors, dimension=16)
...
def test_concurrent_ivf_index_mixed_operations():
    """Test mixed concurrent operations (add, remove, search)."""
    ids = [str(uuid.uuid4()) for _ in range(50)]
    vectors = [[float(i % 8)] * 8 for i in range(50)]

    index = ConcurrentIvfIndex.build(ids, vectors, dimension=8)
...
def test_concurrent_ivf_index_context_manager():
    """Test using the index as a context manager."""
    ids = [str(uuid.uuid4()) for _ in range(50)]
    vectors = [[float(i % 3)] * 3 for i in range(50)]

    with ConcurrentIvfIndex.build(ids, vectors, dimension=3) as index:
        assert len(index) == 50
    # Index should be closed after exiting context


def test_concurrent_ivf_index_with_config():
    ...
    index = ConcurrentIvfIndex.build(ids, vectors, dimension=16, config=config)
