...
def test_concurrent_flat_index_create():
    """Test creating a concurrent flat index."""
...
def test_concurrent_flat_index_add_and_search():
    """Test adding vectors and searching."""
    index = ConcurrentFlatIndex(dimension=3)
...
def test_concurrent_flat_index_remove():
...
def test_concurrent_flat_index_clone():
...
def test_concurrent_flat_index_multi_threaded_insert():
    """Test concurrent inserts from multiple threads."""
    index = ConcurrentFlatIndex(dimension=32)
    num_threads = 4
...
def test_concurrent_flat_index_multi_threaded_search():
    """Test concurrent searches from multiple threads."""
    index = ConcurrentFlatIndex(dimension=16)
...
def test_concurrent_flat_index_mixed_operations():
    """Test mixed concurrent operations (add, remove, search)."""
    index = ConcurrentFlatIndex(dimension=8)
...
def test_concurrent_flat_index_context_manager():
    """Test using the index as a context manager."""
    with ConcurrentFlatIndex(dimension=3) as index:
        index.add(str(uuid.uuid4()), [1.0, 0.0, 0.0])
        assert len(index) == 1
    # Index should be closed after exiting context


def test_concurrent_flat_index_with_config():
    """Test creating index with custom configuration."""
    config = {
        "distance_metric": "Euclidean",
    }

    index = ConcurrentFlatIndex(dimension=16, config=config)
