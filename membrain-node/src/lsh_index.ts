...
/** LSH (Locality-Sensitive Hashing) vector index client. */
export class MembrainLshIndex {
  private handle: any;
  ...
  private fn_lsh_free: any;
  private fn_lsh_add: any;
  private fn_lsh_remove: any;
  private fn_lsh_search: any;
  private fn_lsh_search_with_filter: any;
  private fn_lsh_batch_search: any;
  private fn_lsh_len: any;
  private fn_lsh_dimension: any;
  private fn_lsh_metrics: any;
  ...
  constructor(dimension?: number, config?: LshIndexConfig, libPath?: string) {
    ...
    if (config) {
      const createWithConfig = this.lib.func(
        "memscale_lsh_index_new_with_config",
        voidPtr,
        ["str"],
      );
    ...
    if (!this.handle) {
      ...
      throw new MembrainError(`failed to create LSH index: ${err}`);
    }
  }
  ...
  private bindFunctions(): void {
    ...
    this.fn_lsh_free = lib.func("memscale_lsh_index_free", "void", [voidPtr]);

    this.fn_lsh_add = lib.func("memscale_lsh_index_add", "int32_t", [
      voidPtr, "str", "str",
    ]);
    this.fn_lsh_remove = lib.func("memscale_lsh_index_remove", "int32_t", [
      voidPtr, "str",
    ]);
    this.fn_lsh_search = lib.func("memscale_lsh_index_search", "int32_t", [
      voidPtr, "str", "uint32_t", voidPtrPtr,
    ]);
    this.fn_lsh_search_with_filter = lib.func(
      "memscale_lsh_index_search_with_filter",
      "int32_t",
      [voidPtr, "str", "uint32_t", "str", voidPtrPtr],
    );
    this.fn_lsh_batch_search = lib.func(
      "memscale_lsh_index_batch_search",
      "int32_t",
      [voidPtr, "str", "uint32_t", voidPtrPtr],
    );
    this.fn_lsh_len = lib.func("memscale_lsh_index_len", "int32_t", [
      voidPtr, "int64_t *",
    ]);
    this.fn_lsh_dimension = lib.func(
      "memscale_lsh_index_dimension",
      "int32_t",
      [voidPtr, "int64_t *"],
    );
    this.fn_lsh_metrics = lib.func("memscale_lsh_index_metrics", "int32_t", [
      voidPtr, voidPtrPtr,
    ]);
  }
  ...
  add(id: string, embedding: number[]): void {
    ...
    const code = this.fn_lsh_add(this.handle, id, embJson);
    this.check(code);
  }

  /** Remove a vector from the index by ID. */
  remove(id: string): void {
    const code = this.fn_lsh_remove(this.handle, id);
    this.check(code);
  }
  ...
  search(query: number[], k: number = 10): IndexSearchResult[] {
    ...
    const code = this.fn_lsh_search(this.handle, queryJson, k, buf);
    this.check(code);
  ...
  searchWithFilter(
    ...
    const code = this.fn_lsh_search_with_filter(
  ...
  batchSearch(queries: number[][], k: number = 10): IndexSearchResult[][] {
    ...
    const code = this.fn_lsh_batch_search(this.handle, queriesJson, k, buf);
    this.check(code);
    const { value: json, ptr } = readStringOut(buf);
    try {
      return JSON.parse(json) as IndexSearchResult[][];
  ...
  len(): number {
    ...
    const code = this.fn_lsh_len(this.handle, out);
    this.check(code);
  ...
  dimension(): number {
    const out = newInt64Out();
    const code = this.fn_lsh_dimension(this.handle, out);
    this.check(code);
    return readInt64(out);
  }
  ...
  metrics(): IndexMetrics {
    const buf = newStringOut();
    const code = this.fn_lsh_metrics(this.handle, buf);
    this.check(code);
    const { value: json, ptr } = readStringOut(buf);
    try {
