/**
 * Thread-safe concurrent IVF (Inverted File) vector index client.
 */
 ...
 * ```
 */
export class ConcurrentIvfIndex {
  private handle: any;
  ...
  private fn_concurrent_ivf_free: any;
  private fn_concurrent_ivf_clone: any;
  private fn_concurrent_ivf_add: any;
  private fn_concurrent_ivf_remove: any;
  private fn_concurrent_ivf_search: any;
  private fn_concurrent_ivf_len: any;
  private fn_concurrent_ivf_dimension: any;
   ...
   * @returns A new ConcurrentIvfIndex instance
   */
  static build(
    ids: string[],
    vectors: number[][],
    ...
    const buildFunc = lib.func(
      "memscale_concurrent_ivf_index_build",
      voidPtr,
    ...
    if (!handle) {
      ...
      throw new MembrainError(`failed to build concurrent IVF index: ${err}`);
    }

    return new ConcurrentIvfIndex(handle, lib);
  }
  ...
  private bindFunctions(): void {
    ...
    this.fn_concurrent_ivf_free = lib.func("memscale_concurrent_ivf_index_free", "void", [voidPtr]);
    this.fn_concurrent_ivf_clone = lib.func("memscale_concurrent_ivf_index_clone", voidPtr, [voidPtr]);

    this.fn_concurrent_ivf_add = lib.func("memscale_concurrent_ivf_index_add", "int32_t", [
      voidPtr, "str", "float *", "uint32_t",
    ]);
    this.fn_concurrent_ivf_remove = lib.func("memscale_concurrent_ivf_index_remove", "int32_t", [
      voidPtr, "str", "int32_t *",
    ]);
    this.fn_concurrent_ivf_search = lib.func("memscale_concurrent_ivf_index_search", "int32_t", [
      voidPtr, "float *", "uint32_t", "uint32_t", voidPtrPtr,
    ]);
    this.fn_concurrent_ivf_len = lib.func("memscale_concurrent_ivf_index_len", "int32_t", [
      voidPtr, "int64_t *",
    ]);
    this.fn_concurrent_ivf_dimension = lib.func(
      "memscale_concurrent_ivf_index_dimension",
      "int32_t",
      [voidPtr, "int64_t *"],
    );
  }
   ...
   *
   * @returns A new ConcurrentIvfIndex instance sharing the same underlying index
   */
  clone(): ConcurrentIvfIndex {
    const clonedHandle = this.fn_concurrent_ivf_clone(this.handle);
    if (!clonedHandle) {
      const err = this.getLastError();
      throw new MembrainError(`failed to clone concurrent IVF index: ${err}`);
    }
    return new ConcurrentIvfIndex(clonedHandle, this.lib);
  }
  ...
  add(id: string, embedding: number[]): void {
    ...
    const code = this.fn_concurrent_ivf_add(
      this.handle,
      id,
      vectorArray,
      embedding.length,
  ...
  remove(id: string): boolean {
    ...
    const code = this.fn_concurrent_ivf_remove(this.handle, id, out);
    this.check(code);
    return out[0] !== 0;
  }
  ...
  search(query: number[], k: number = 10): IndexSearchResult[] {
    ...
    const code = this.fn_concurrent_ivf_search(
      this.handle,
      queryArray,
      query.length,
      k,
  ...
  len(): number {
    ...
    const code = this.fn_concurrent_ivf_len(this.handle, out);
    this.check(code);
  ...
  dimension(): number {
    const out = newInt64Out();
    const code = this.fn_concurrent_ivf_dimension(this.handle, out);
    this.check(code);
    return readInt64(out);
  }
  ...
  close(): void {
    if (this.handle) {
      this.fn_concurrent_ivf_free(this.handle);
      this.handle = null;
    }
  }
}
