...
import { ConcurrentVamanaIndex } from "../dist/index.js";
...
function buildIndex(count = 100, config) {
  const ids = Array.from({ length: count }, () => randomUUID());
  const vectors = ids.map(() => randomVector());
  return {
    index: ConcurrentVamanaIndex.build(ids, vectors, DIMENSION, config),
    ids,
    vectors,
  };
}

describe("ConcurrentVamanaIndex", () => {
