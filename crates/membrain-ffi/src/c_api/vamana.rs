...
use memscaledb::{BatchSearch, VamanaConfig, VamanaIndex, VectorId, VectorIndex};
...
const NULL_MSG: &str = "null Vamana index pointer";

#[inline]
unsafe fn get_vamana_index<'a>(index: *const VamanaIndex) -> Result<&'a VamanaIndex, i32> {
    // SAFETY: delegates to `as_ref_or`; caller contract carries forward.
    unsafe { as_ref_or(index, NULL_MSG) }
}

#[inline]
unsafe fn get_vamana_index_mut<'a>(index: *mut VamanaIndex) -> Result<&'a mut VamanaIndex, i32> {
...
///
/// `config_json` is a JSON string with Vamana index configuration. Must include "dimension".
/// `ids_json` is a JSON array of UUID strings.
/// `vectors_json` is a JSON array of float arrays.
///
/// Config JSON format:
///   {"dimension":128,"max_degree":64,"alpha":1.2,"search_list_size":128,
...
#[no_mangle]
pub unsafe extern "C" fn memscale_vamana_index_build(
    ...
    unsafe {
        ...
        struct VamanaCreateConfig {
            dimension: usize,
            #[serde(flatten)]
            vamana: VamanaConfig,
        }

        let parsed: VamanaCreateConfig = match serde_json::from_str(config_str) {
            Ok(c) => c,
            Err(e) => {
                set_last_error(&format!("invalid Vamana index config JSON: {}", e));
                return ptr::null_mut();
            }
        };
        ...
        match VamanaIndex::build(&ids, &vector_refs, parsed.dimension, parsed.vamana) {
            Ok(index) => Box::into_raw(Box::new(index)),
            Err(e) => {
                set_last_error(&format!("failed to build Vamana index: {}", e));
                ptr::null_mut()
            }
        }
    }
...
/// Destroy a Vamana index and free all associated resources.
/// Passing NULL is a no-op.
///
/// # Safety
///
/// - `index` must have been returned by `memscale_vamana_index_build`, and must
///   not have been freed already, or be null.
#[no_mangle]
pub unsafe extern "C" fn memscale_vamana_index_free(index: *mut VamanaIndex) {
...
/// Add a vector to the Vamana index.
///
/// # Safety
///
/// - `index` must be a valid pointer returned by `memscale_vamana_index_build`.
/// - `id` and `embedding_json` must be non-null and point to valid
///   null-terminated C strings.
#[no_mangle]
pub unsafe extern "C" fn memscale_vamana_index_add(
    ...
    // doc. Each unsafe call within this block relies on the same contract.
    unsafe {
        let index = match get_vamana_index_mut(index) {
            Ok(i) => i,
...
/// Remove a vector from the Vamana index.
///
/// # Safety
///
/// - `index` must be a valid pointer returned by `memscale_vamana_index_build`.
...
#[no_mangle]
pub unsafe extern "C" fn memscale_vamana_index_remove(
    index: *mut VamanaIndex,
    id: *const c_char,
) -> i32 {
    // SAFETY: raw-pointer preconditions are stated in the fn-level `# Safety`
...
/// Search the Vamana index for nearest neighbors.
///
/// # Safety
///
/// - `index` must be a valid pointer returned by `memscale_vamana_index_build`.
...
#[no_mangle]
pub unsafe extern "C" fn memscale_vamana_index_search(
    index: *const VamanaIndex,
    ...
    // doc. Each unsafe call within this block relies on the same contract.
    unsafe {
        let index = match get_vamana_index(index) {
            Ok(i) => i,
            Err(code) => return code,
        };
...
/// Search the Vamana index with an ID filter.
///
/// # Safety
///
/// - `index` must be a valid pointer returned by `memscale_vamana_index_build`.
...
/// - `out_json` must be null or point to valid writable memory.
#[no_mangle]
pub unsafe extern "C" fn memscale_vamana_index_search_with_filter(
    index: *const VamanaIndex,
    query_json: *const c_char,
    k: u32,
...
/// - `out_json` must be null or point to valid writable memory.
#[no_mangle]
pub unsafe extern "C" fn memscale_vamana_index_batch_search(
    index: *const VamanaIndex,
...
/// Get the number of vectors in the Vamana index.
///
/// # Safety
///
/// - `index` must be a valid pointer returned by `memscale_vamana_index_build`.
...
#[no_mangle]
pub unsafe extern "C" fn memscale_vamana_index_len(
    index: *const VamanaIndex,
    out_count: *mut i64,
) -> i32 {
    // SAFETY: raw-pointer preconditions are stated in the fn-level `# Safety`
...
/// Get the vector dimension of the Vamana index.
///
/// # Safety
///
/// - `index` must be a valid pointer returned by `memscale_vamana_index_build`.
...
#[no_mangle]
pub unsafe extern "C" fn memscale_vamana_index_dimension(
    index: *const VamanaIndex,
...
/// Get Vamana index performance metrics as JSON.
///
/// # Safety
///
/// - `index` must be a valid pointer returned by `memscale_vamana_index_build`.
/// - `out_json` must be null or point to valid writable memory.
#[no_mangle]
pub unsafe extern "C" fn memscale_vamana_index_metrics(
    index: *const VamanaIndex,
    out_json: *mut *mut c_char,
) -> i32 {
    // SAFETY: raw-pointer preconditions are stated in the fn-level `# Safety`
