...
use memscaledb::{ConcurrentIvfIndex, ConcurrentVectorIndex, IvfConfig, VectorId};
...
// ---------------------------------------------------------------------------
// ConcurrentIvfIndex (thread-safe IVF/k-means partitioned index)
// ---------------------------------------------------------------------------

#[inline]
unsafe fn get_concurrent_ivf_index<'a>(
    index: *const Arc<ConcurrentIvfIndex>,
) -> Result<&'a Arc<ConcurrentIvfIndex>, i32> {
    // SAFETY: delegates to `as_ref_or`; caller contract carries forward.
    unsafe { as_ref_or(index, "null concurrent IVF index pointer") }
}

/// Build a new thread-safe concurrent IVF index from training data.
...
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_ivf_index_build(
    ids_data: *const *const c_char,
    vectors_data: *const f32,
    num_vectors: u32,
    dimension: u32,
    config_json: *const c_char,
) -> *mut Arc<ConcurrentIvfIndex> {
    ...
    unsafe {
        ...
        let config = if config_json.is_null() {
            ...
            match serde_json::from_str(json_str) {
                Ok(c) => c,
                Err(e) => {
                    set_last_error(&format!("invalid IVF config JSON: {}", e));
                    return ptr::null_mut();
                }
            }
        };

        match ConcurrentIvfIndex::build(&ids, &vector_refs, dimension as usize, config) {
            Ok(index) => Box::into_raw(Box::new(Arc::new(index))),
            Err(e) => {
                set_last_error(&format!("failed to build concurrent IVF index: {}", e));
                ptr::null_mut()
            }
        }
    }
...
/// Clone a concurrent IVF index handle for use in another thread.
///
/// # Safety
///
/// - `index` must be a valid pointer returned by `memscale_concurrent_ivf_index_build`.
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_ivf_index_clone(
    index: *const Arc<ConcurrentIvfIndex>,
) -> *mut Arc<ConcurrentIvfIndex> {
    // SAFETY: raw-pointer preconditions are stated in the fn-level `# Safety`
    // doc. Each unsafe call within this block relies on the same contract.
    unsafe {
        let arc = match get_concurrent_ivf_index(index) {
            Ok(a) => a,
            Err(_) => return ptr::null_mut(),
        };
...
/// Free a concurrent IVF index handle.
///
/// # Safety
///
/// - `index` must be a valid pointer.
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_ivf_index_free(index: *mut Arc<ConcurrentIvfIndex>) {
...
/// Add a vector to the concurrent IVF index (thread-safe).
///
/// # Safety
///
/// - `index` must be a valid pointer.
...
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_ivf_index_add(
    index: *const Arc<ConcurrentIvfIndex>,
    id_str: *const c_char,
...
/// Remove a vector from the concurrent IVF index (thread-safe).
///
/// # Safety
///
/// - `index` must be a valid pointer.
/// - `id_str` must be a valid null-terminated C string.
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_ivf_index_remove(
    index: *const Arc<ConcurrentIvfIndex>,
    id_str: *const c_char,
    out_found: *mut i32,
) -> i32 {
...
/// Search the concurrent IVF index (thread-safe).
///
/// # Safety
///
/// - `index` must be a valid pointer.
...
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_ivf_index_search(
    index: *const Arc<ConcurrentIvfIndex>,
...
/// Get the number of vectors in the concurrent IVF index (thread-safe).
///
/// # Safety
///
/// - `index` must be a valid pointer.
...
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_ivf_index_len(
    index: *const Arc<ConcurrentIvfIndex>,
    out_count: *mut i64,
) -> i32 {
    // SAFETY: raw-pointer preconditions are stated in the fn-level `# Safety`
...
/// Get the dimension of vectors in the concurrent IVF index.
///
/// # Safety
///
/// - `index` must be a valid pointer.
...
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_ivf_index_dimension(
    index: *const Arc<ConcurrentIvfIndex>,
