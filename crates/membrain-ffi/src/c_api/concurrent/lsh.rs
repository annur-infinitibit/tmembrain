...
use memscaledb::{ConcurrentLshIndex, ConcurrentVectorIndex, LshConfig, VectorId};
...
// ---------------------------------------------------------------------------
// ConcurrentLshIndex (thread-safe LSH/locality-sensitive hashing index)
// ---------------------------------------------------------------------------

#[inline]
unsafe fn get_concurrent_lsh_index<'a>(
    index: *const Arc<ConcurrentLshIndex>,
) -> Result<&'a Arc<ConcurrentLshIndex>, i32> {
    // SAFETY: delegates to `as_ref_or`; caller contract carries forward.
    unsafe { as_ref_or(index, "null concurrent LSH index pointer") }
}

/// Create a new thread-safe concurrent LSH index.
///
/// # Safety
///
/// - No special requirements.
#[no_mangle]
pub extern "C" fn memscale_concurrent_lsh_index_new(
    dimension: u32,
) -> *mut Arc<ConcurrentLshIndex> {
    let index = ConcurrentLshIndex::new(dimension as usize);
    Box::into_raw(Box::new(Arc::new(index)))
}

/// Create a new thread-safe concurrent LSH index with custom configuration.
///
/// # Safety
///
/// - `config_json` must be null or point to a valid null-terminated C string.
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_lsh_index_new_with_config(
    dimension: u32,
    ...
    unsafe {
        ...
        let index = ConcurrentLshIndex::with_config(dimension as usize, config);
        Box::into_raw(Box::new(Arc::new(index)))
    }
}

/// Clone a concurrent LSH index handle for use in another thread.
///
/// # Safety
///
/// - `index` must be a valid pointer returned by `memscale_concurrent_lsh_index_new`.
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_lsh_index_clone(
    index: *const Arc<ConcurrentLshIndex>,
) -> *mut Arc<ConcurrentLshIndex> {
    // SAFETY: raw-pointer preconditions are stated in the fn-level `# Safety`
    // doc. Each unsafe call within this block relies on the same contract.
    unsafe {
        let arc = match get_concurrent_lsh_index(index) {
            Ok(a) => a,
            Err(_) => return ptr::null_mut(),
        };
...
/// Free a concurrent LSH index handle.
///
/// # Safety
///
/// - `index` must be a valid pointer.
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_lsh_index_free(index: *mut Arc<ConcurrentLshIndex>) {
...
/// Add a vector to the concurrent LSH index (thread-safe).
///
/// # Safety
///
/// - `index` must be a valid pointer.
...
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_lsh_index_add(
    index: *const Arc<ConcurrentLshIndex>,
    id_str: *const c_char,
...
/// Remove a vector from the concurrent LSH index (thread-safe).
///
/// # Safety
///
/// - `index` must be a valid pointer.
/// - `id_str` must be a valid null-terminated C string.
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_lsh_index_remove(
    index: *const Arc<ConcurrentLshIndex>,
    id_str: *const c_char,
    out_found: *mut i32,
) -> i32 {
...
/// Search the concurrent LSH index (thread-safe).
///
/// # Safety
///
/// - `index` must be a valid pointer.
...
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_lsh_index_search(
    index: *const Arc<ConcurrentLshIndex>,
...
/// Get the number of vectors in the concurrent LSH index (thread-safe).
///
/// # Safety
///
/// - `index` must be a valid pointer.
...
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_lsh_index_len(
    index: *const Arc<ConcurrentLshIndex>,
    out_count: *mut i64,
) -> i32 {
    // SAFETY: raw-pointer preconditions are stated in the fn-level `# Safety`
...
/// Get the dimension of vectors in the concurrent LSH index.
///
/// # Safety
///
/// - `index` must be a valid pointer.
...
#[no_mangle]
pub unsafe extern "C" fn memscale_concurrent_lsh_index_dimension(
    index: *const Arc<ConcurrentLshIndex>,
